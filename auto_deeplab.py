...
class AutoDeeplab (nn.Module):
    def __init__(self, num_classes, num_layers, criterion, multiplier=8, steps=5, cell=model_search.Cell):
        ...
        self.stem1 = nn.Sequential(
            nn.Conv2d(self._num_channels[0], self._num_channels[0], 3, padding=1),
            nn.BatchNorm2d(64)
        )
        ...
        for i in range(self._num_layers):
            if i == 0:
                ...
                cell1_1 = cell(self._steps, self._multiplier, self._num_channels[1], self._num_channels[1], self._num_channels[1], 1)
                cell1_2 = cell(self._steps, self._multiplier, self._num_channels[1], self._num_channels[2], self._num_channels[1], 0)
                cell2_1 = cell(self._steps, self._multiplier, self._num_channels[2], self._num_channels[1], self._num_channels[2], 2)
                cell2_2 = cell(self._steps, self._multiplier, self._num_channels[2], self._num_channels[2], self._num_channels[2], 1)
                cell2_3 = cell(self._steps, self._multiplier, self._num_channels[2], self._num_channels[3], self._num_channels[2], 0)
                cell3_1 = cell(self._steps, self._multiplier, -1, self._num_channels[2], self._num_channels[3], 2)
                cell3_2 = cell(self._steps, self._multiplier, -1, self._num_channels[3], self._num_channels[3], 1)
                cell4   = cell(self._steps, self._multiplier, -1, self._num_channels[3], self._num_channels[4], 2)
        ...
        self.aspp_4 = nn.Sequential(
            ASPP(self._num_channels[1], 256, 24, 24)
        )
        self.aspp_8 = nn.Sequential(
            ASPP(self._num_channels[2], 256, 12, 12)
        )
        self.aspp_16 = nn.Sequential(
            ASPP(self._num_channels[3], 256, 6, 6)
        )
        self.aspp_32 = nn.Sequential(
            ASPP(self._num_channels[4], 256, 3, 3)
        )
    ...
    def forward(self, x):
        ...
        self.level_16 = []
        self.level_32 = []
        self.level_2.append(self.stem1(x))
        ...
        w_alpha = [F.softmax(p[1], 1) for p in self.named_parameters() if p[0].find('w_alpha') > 0]
        w_beta = [F.softmax(p[1], 0) for p in self.named_parameters() if p[0].find('w_beta') > 0]

        for layer in range(self._num_layers):
            if layer == 0:
                level4_new = self.cells[count](None, self.level_4[-1], w_alpha)
                count += 1
                level8_new = self.cells[count](None, self.level_4[-1], w_alpha)
                count += 1
                self.level_4.append(level4_new * self.w_beta[0][0])
                self.level_8.append(level8_new * self.w_beta[0][1])
                # print ((self.level_4[-2]).size (),  (self.level_4[-1]).size())
            elif layer == 1:
                level4_new_1 = self.cells[count](self.level_4[-2], self.level_4[-1], w_alpha)
                count += 1
                level4_new_2 = self.cells[count](self.level_4[-2], self.level_8[-1], w_alpha)
                count += 1
                level4_new = self.w_beta[1][0] * level4_new_1 + self.w_beta[2][0] * level4_new_2

                level8_new_1 = self.cells[count](None, self.level_4[-1], w_alpha)
                count += 1
                level8_new_2 = self.cells[count](None, self.level_8[-1], w_alpha)
                count += 1
                level8_new = self.w_beta[1][1] * level8_new_1 + self.w_beta[layer][2][1] * level8_new_2

                level16_new = self.cells[count](None, self.level_8[-1], w_alpha)
                level16_new = level16_new * self.w_beta[2][2]
                count += 1
                ...
                level4_new_2 = self.cells[count](self.level_4[-2], self.level_8[-1], w_alpha)
                count += 1
                level4_new = self.w_beta[3][0] * level4_new_1 + self.w_beta[4][1] * level4_new_2

                level8_new_1 = self.cells[count](self.level_8[-2], self.level_4[-1], w_alpha)
                count += 1
                level8_new_2 = self.cells[count](self.level_8[-2], self.level_8[-1], w_alpha)
                count += 1
                level8_new_3 = self.cells[count](self.level_8[-2], self.level_16[-1], w_alpha)
                count += 1
                level8_new = self.w_beta[3][1] * level8_new_1 + self.w_beta[4][1] * level8_new_2 + self.w_beta[5][0] * level8_new_3

                level16_new_1 = self.cells[count](None, self.level_8[-1], w_alpha)
                count += 1
                level16_new_2 = self.cells[count](None, self.level_16[-1], w_alpha)
                count += 1
                level16_new = self.w_beta[4][2] * level16_new_1 + self.w_beta[5][1] * level16_new_2

                level32_new = self.cells[count](None, self.level_16[-1], w_alpha)
                level32_new = level32_new * self.w_beta[5][2]
                count += 1
                ...
                level4_new_2 = self.cells[count](self.level_4[-2], self.level_8[-1], w_alpha)
                count += 1
                level4_new = self.w_beta[6][0] * level4_new_1 + self.w_beta[7][0] * level4_new_2
                ...
                level8_new_3 = self.cells[count](self.level_8[-2], self.level_16[-1], w_alpha)
                count += 1
                level8_new = self.w_beta[6][1] * level8_new_1 + self.w_beta[7][1] * level8_new_2 + self.w_beta[8][0] * level8_new_3

                level16_new_1 = self.cells[count](self.level_16[-2], self.level_8[-1], w_alpha)
                count += 1
                level16_new_2 = self.cells[count](self.level_16[-2], self.level_16[-1], w_alpha)
                count += 1
                level16_new_3 = self.cells[count](self.level_16[-2], self.level_32[-1], w_alpha)
                count += 1
                level16_new = self.w_beta[7][2] * level16_new_1 + self.w_beta[8][1] * level16_new_2 + self.w_beta[9][0] * level16_new_3

                level32_new_1 = self.cells[count](None, self.level_16[-1], w_alpha)
                count += 1
                level32_new_2 = self.cells[count](None, self.level_32[-1], w_alpha)
                count += 1
                level32_new = self.w_beta[8][2] * level32_new_1 + self.w_beta[9][1] * level32_new_2
                ...
                level4_new_2 = self.cells[count](self.level_4[-2], self.level_8[-1], w_alpha)
                count += 1
                level4_new = self.w_beta[layer*4-6][0] * level4_new_1 + self.w_beta[layer*4-5][0] * level4_new_2
                ...
                level8_new_3 = self.cells[count](self.level_8[-2], self.level_16[-1], w_alpha)
                count += 1
                level8_new = self.w_beta[layer*4-6][1] * level8_new_1 + self.w_beta[layer*4-5][1] * level8_new_2 + self.w_beta[layer*4-4][0] * level8_new_3
                ...
                level16_new_3 = self.cells[count](self.level_16[-2], self.level_32[-1], w_alpha)
                count += 1
                level16_new = self.w_beta[layer*4-5][2] * level16_new_1 + self.w_beta[layer*4-4][1] * level16_new_2 + self.w_beta[layer*4-3][0] * level16_new_3


                level32_new_1 = self.cells[count](self.level_32[-2], self.level_16[-1], w_alpha)
                count += 1
                level32_new_2 = self.cells[count](self.level_32[-2], self.level_32[-1], w_alpha)
                count += 1
                level32_new = self.w_beta[layer*4-4][2] * level32_new_1 + self.w_beta[layer*4-3][1] * level32_new_2
